...
public extension UIView {
    ...
    func makeToast(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, title: String? = nil, image: UIImage? = nil, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
        do {
            ...
            showToast(toast, toastFrame: toast.frame.size, duration: duration, position: position, completion: completion)
    ...
    /**
     ...
     didTap will be `true` if the toast view was dismissed from a tap.
     */
    func showToast(_ toast: UIView, toastFrame: CGSize, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, completion: ((_ didTap: Bool) -> Void)? = nil) {
        let point = position.centerPoint(forToast: toast, toastFrame: toastFrame, inSuperview: self)
        showToast(toast, duration: duration, point: point, completion: completion)
    }
    ...
    */
    ...
    func makeToastActivity(_ position: ToastPosition) {
        ...
        let point = position.centerPoint(forToast: toast, toastFrame: toast.frame.size, inSuperview: self)
...
public enum ToastPosition {
    ...
    fileprivate func centerPoint(forToast toast: UIView, toastFrame: CGSize = .zero, inSuperview superview: UIView) -> CGPoint {
        ...
        let toastHeight: CGFloat = toast.frame.size.height == 0.0 ? toastFrame.height : toast.frame.size.height
        
        switch self {
        case .top:
            return CGPoint(x: superview.bounds.size.width / 2.0, y: (toastHeight / 2.0) + topPadding)
        case .center:
            return CGPoint(x: superview.bounds.size.width / 2.0, y: superview.bounds.size.height / 2.0)
        case .bottom:
            return CGPoint(x: superview.bounds.size.width / 2.0, y: (superview.bounds.size.height - (toastHeight / 2.0)) - bottomPadding)
        }
    }
}
